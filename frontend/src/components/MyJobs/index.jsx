...
const MyJobs = () =>{
    ...
    const handleDecline = async (proposalId) => {
        ...
        try{
            const response = await axios.post(`http://localhost:8080/api/proposal/delete`, data);
    ...
    const handleViewProposal = async ({email, proposalId}) => {
        ...
        try {
          // Fetching username
          const usernameResponse = await axios.get('http://localhost:8080/api/getUsername/userposted', { params: data });
          const { name } = usernameResponse.data;
          setUsername(name);
          console.log(name);
      
          // Fetching user profile
          const userProfileResponse = await axios.get('http://localhost:8080/api/userprofile/find', { params: data });
          setUserProfile(userProfileResponse.data);
          console.log(userprofile);
      
          // Fetching user info
          const userInfoResponse = await axios.get('http://localhost:8080/api/user_info/find', { params: data });
          setUserInfo(userInfoResponse.data);
          console.log(userinfo);

          //set proposal by index
          const proposal = proposals.find((proposal) => proposal._id === proposalId);
                ...
                <div className="w-full md:w-1/2 px-4 py-3">
                    ...
                    <div className="bg-white rounded-lg shadow p-4 mb-4" key={job._id}>
                        ...
                        <div className="w-full flex items-center justify-center">
                            <button onClick={() => handleBtnClick({job, index})}
                        ...
                        >
                            <div className="mt-2 w-full items-center justify-center ">
                                ...
                                <div className="w-full">
                                    <h2 className="text-lg font-bold">{(proposals.length>0)? "Proposals" : "No proposals received for this job"}</h2>
                                    ...
                                    <div className="flex items-center justify-between my-4 p-4 bg-gradient-to-r from-neutral-900 to-zinc-700 rounded-lg shadow-md">
                                        ...
                                        <div className="flex flex-row">
                                            ...
                                            {isLoading ? (
                                                <>
                                                <button
                                                    onClick={() => handleViewProposal({ email: proposal.senderEmail, proposalId: proposal._id })}
                                                    ...
                                                    >
                                                <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-white border-r-transparent align-[-0.125em] text-secondary motion-reduce:animate-[spin_1.5s_linear_infinite]" role="status">
                                                ...
                                                <div className="bg-stone-900  rounded-lg shadow-lg p-6 w-full">
                                                    ...
                                                    <div className="flex flex-col w-full">
                                                        <div className="rounded-lg border-2 border-white pl-3 py-1">
                                                            <h1 className="text-lg font-medium text-white">Job: {job.jobName}</h1>
                                                        </div>
                                                        ...
                                                        <div className="mt-3 flex flex-row w-full">
                                                            <div className="rounded-lg border-2 border-white w-2/5 mr-1">
                                                                ...
                                                                <div className="py-2 pl-3 pr-2">
                                                                    <h1 className="text-base font-medium text-white">Name: {username}</h1>
                                                                    <h2 className="text-base font-normal text-white">Email: {userinfo.email}</h2>
                                                                    <h2 className="text-base font-normal text-white">Category: {userinfo.usercategory}</h2>
                                                                    <h2 className="text-base font-normal text-white">Department: {userinfo.department}</h2>

                                                                    <div className="mt-2 ">
                                                                    <h3 className="text-lg font-medium text-white">Achievements:</h3>
                                                                    <p className="text-base text-white">{userprofile && userprofile.Achievements ? userprofile.Achievements : "Profile not completed"}</p>
                                                                    <h3 className="text-lg font-medium text-white">Education:</h3>
                                                                    <p className="text-base text-white">{userprofile && userprofile.Education ? userprofile.Education : "Profile not completed"}</p>
                                                                    <h3 className="text-lg font-medium text-white">Strength:</h3>
                                                                    <p className="text-base text-white">{userprofile && userprofile.Strength ? userprofile.Strength : "Profile not completed"}</p>
                                                                    <h3 className="text-lg font-medium text-white">Experience:</h3>
                                                                    <p className="text-base text-white">{userprofile && userprofile.Experience ? userprofile.Experience : "Profile not completed"}</p>
                                                            ...
                                                            <div className="rounded-lg border-2 border-white w-3/5 ml-1 flex flex-col">
                                                                <div className="bg-stone-900 py-1 pl-3 rounded-lg">
                                                                    <h1 className="text-lg font-medium text-white">Proposal</h1>
                                                                </div>
                                                                <div className="bg-gray-100 flex-grow  ">
                                                                    <div className="pl-2 pt-2">
                                                                        {
                                                                            (job.jobDuration === "Hourly") ? (
                                                                                <h1 className="text-normal font-bold">Proposed rate: {thisProposal.proposedPrice} \hr</h1>
                                                                            ): (
                                                                                <h1 className="text-normal font-bold">Proposed price: {thisProposal.proposedPrice}</h1>
                                                                            )
                                                                        }
                                                                    </div>
                                                                    <div className="pl-2 pt-2">
                                                                        ...
                                                                        <div className="overflow-auto max-h-96">
                                                                            <pre className="whitespace-pre-wrap">
                                                                                <p className="">{thisProposal.coverLetter}</p>
                                                                            </pre>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                    ...
                                                    <div className="flex justify-end">
                                                        <button onClick={() => setIsModalVisible(false)} className="block mt-4 mx-3 w-full md:w-auto inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-gradient-to-r from-green-500 to-teal-500 hover:bg-gradient-to-r hover:from-green-400 hover:to-teal-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 shadow-sm">
                                                            Go back
                                                        </button>
                                                        <button 
                                                        onClick = {() =>{handleDecline(thisProposal._id)}}
